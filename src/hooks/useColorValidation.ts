...
import { validateColorName, validateRgbValue } from "../validators/colorValidators";
...
interface UseColorValidationReturn {
  ...
  validateField: (field: keyof ValidationErrors, value: string) => string | undefined;
...
export function useColorValidation(): UseColorValidationReturn {
  ...
  // Set specific error
  const setError = useCallback((field: keyof ValidationErrors, error: string) => {
    setErrors(prev => ({
      ...prev,
      [field]: error,
    }));
